...
> 将以下内容添加到项目的 CLAUDE.md 中，作为 L2 协议门禁注入。
...
- 每次 intake 完成后，调用 `harness state init` 初始化任务状态
- 每次状态变更后，调用 `harness state update` 更新持久化文件
- 任务恢复时，调用 `harness state restore` 读取上次状态
- 状态文件位置：`harness/state/tasks/<task_id>.json`
