...
- `@brawnen/agent-harness-cli` 通过 npm 显式依赖 `@brawnen/agent-harness-protocol`，而不是在 CLI 包里复制一份 protocol 资源
...
- 正面影响：
  - npm 发布边界更清晰，安装 `@brawnen/agent-harness-cli` 时会自动带上 protocol
